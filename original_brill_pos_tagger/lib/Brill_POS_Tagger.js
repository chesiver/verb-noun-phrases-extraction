...
function Brill_POS_Tagger(lexicon, ruleSetCurrent, ruleSetConditional) {
  this.lexicon = lexicon;
  this.ruleSetCurrent = ruleSetCurrent;
  this.ruleSetConditional = ruleSetConditional;
}
...
Brill_POS_Tagger.prototype.tag = function(sentence) {
  ...
  return this.applyRules(taggedSentence, sentence);
};
...
Brill_POS_Tagger.prototype.applyRules = function (taggedSentence, sentence) {
  for (var i = 0, size = taggedSentence.taggedWords.length; i < size; i++) {
    var isExist = this.lexicon.isExist(sentence[i]);
    this.ruleSetCurrent.getRules().forEach(function (rule) {
      if (isExist) {
        var categories = this.lexicon.tagWord(sentence[i]);
        if (categories && categories.includes(rule.new_category)) {
  ...
  for (var i = 0, size = taggedSentence.taggedWords.length; i < size; i++) {
    this.ruleSetConditional.getRules().forEach(function (rule) { 
      rule.apply(taggedSentence, i);
    })
  }
  return taggedSentence;
